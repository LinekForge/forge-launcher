...
- 设置面板增加**默认历史条数**——通道弹窗的初始回放条数（替代硬编码 100）

### Bug 修复
- 预设通道列表现在也走 enabledChannels 过滤
- "全通道"标签在通道被过滤时改为"已启用通道"
- 所有通道被禁用时 beep 提示（不再静默无响应）
- count=0 行为在启动路径和弹窗路径之间统一
- combo 输入 fallback 使用 config.defaultHistoryCount（不再硬编码 100）
- build.sh 编译完自动重开 app
